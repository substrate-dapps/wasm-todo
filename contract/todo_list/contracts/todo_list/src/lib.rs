...
mod todo_list {
    ...
    // Default implementation for todo item
    ...
    impl TodoList {
        ...
        #[ink(message)]
        pub fn update_todo_item(&mut self, item_id: TodoItemId) -> Result<(), TodoError> {
        ...
        pub fn remove_todo_item(&mut self, item_id: TodoItemId) -> Result<(), TodoError> {
            ...
            if item.owner == self.env().caller() {
                self.item.remove(item_id);
                self.env().emit_event(ItemRemoved { item_id });
            } else {
                return Err(TodoError::NotAOwner);
            }
            Ok(())
        ...
        #[ink(message)]
        pub fn get_todo_length(&self) -> i32 {
            let mut item: Vec<TodoItem> = Vec::new();
            for _item in 0..self.item_id {
                match self.item.get(_item) {
                    Some(value) => {
    ...
    // Define zero address for `AccountId`
    // fn zero_address() -> AccountId {
    //     [0u8; 32].into()
    // }

    // Unit test for Todo List Applications
    #[cfg(test)]
    mod test {
        use super::*;
        ...
        fn create_todo_item_works() {
            ...
            contract
                .create_todo_item(name, Prioritise::HIGH)
                .unwrap_or_default();
            ...
            contract
                .create_todo_item(name, Prioritise::LOW)
                .unwrap_or_default();
        ...
        pub fn update_todo_item() {
            ...
            contract.update_todo_item(1).unwrap_or_default();
