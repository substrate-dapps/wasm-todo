...
export default class Constructors {
	...
	*/
   	async "new" (
   		...
   		const __contract = JSON.parse(Files.readFileSync("./artifacts/voting_contract.contract").toString());
		const code = new CodePromise(this.nativeAPI, __contract, __contract.source.wasm);
		const gasLimit = 100000 * 1000000 || __options?.gasLimit;

		const storageDepositLimit = __options?.storageDepositLimit;
        const tx = code.tx["new"]!({ gasLimit, storageDepositLimit, value: __options?.value }, initialValue);
		let response;
